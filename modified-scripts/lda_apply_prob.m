...
% P = [pdf('Normal',X_LDA,class_mv(1,1), class_mv(2,1)^2 + class_mv(2,2)^2) pdf('Normal',X_LDA,class_mv(1,2), class_mv(2,1)^2 + class_mv(2,2)^2)];

P = P./repmat(sum(P,2),1,2);
predicted_y = sign(P(:,1)-P(:,2));

% predict by the sign of the projection
predicted_y = sign(X * w + B);
